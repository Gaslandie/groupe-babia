...
| 2026-09-17 | Adresse officielle : `contact@groupebabia.com` partout, en remplacement de `infobabiaguinee@gmail.com`. | Demande de Gassama. Une adresse au nom du domaine vaut mieux qu'une adresse Gmail pour un groupe qui s'adresse a des acheteurs internationaux. | Remplacee dans l'affichage (liens mailto, pages legales, donnees structurees) et dans les valeurs par defaut du code (`app/config.php`, `contact-submit.php`, `.env.example`). **Deux reserves** : le `.env` du serveur peut porter `CONTACT_RECIPIENT_EMAIL` et ecraser la valeur par defaut ; et la boite `contact@` doit exister chez OVH, sinon les notifications rebondissent. Les demandes restent enregistrees en base et consultables dans `espace-gb/messages.php` : aucune demande n'est perdue meme si l'e-mail echoue. |
| 2026-09-17 | Corriger trois defauts constates sur telephone en production. | Captures de Gassama : titres et descriptions invisibles dans les cartes produit, vignettes du mur produits en portrait geant, page suivante affichee alors que l'ancienne restait a sa position de defilement. | 1) `.dark-section` imposait du texte blanc aux cartes a fond blanc : regles de rappel ajoutees pour `product-card`, `news-card` et `activity-card`. 2) L'attribut HTML `height` agit comme une regle de presentation et l'emportait sur `aspect-ratio` : `height: auto` ajoute sur les vignettes. 3) `window.scrollTo(0, 0)` avant chaque navigation interne, saut instantane car la navigation n'est pas interceptee. |
...
- ~~Quelle adresse e-mail officielle doit remplacer les contacts incoherents ?~~ Tranche le 2026-09-17 : `contact@groupebabia.com`. Reste a confirmer que cette boite existe bien chez OVH (formule MX Plan, une seule boite).
